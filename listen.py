...
ML1_GAP_S = float(os.getenv("ML1_GAP_S", "1.0"))
ML2_GAP_S = float(os.getenv("ML2_GAP_S", "0.25"))
...
current = {
    ...
    "amount": base_amount, "chain_id": None, "expected_close": None
}
seen_ids = set()
executor_busy = False
scheduled_tasks, ml1_task, ml2_task = [], None, None
...
def quick_peek(chain_id: str):
    try:
        ...
        if r.status_code == 200:
            j = r.json()
            return bool(j.get("ok")), float(j.get("profit", 0.0)), j.get("ml_tag",""), j.get("chain_id",""), j.get("closed_at")
    except: pass
    return False, 0.0, "", "", None

def executor_trade(pair, amount, direction, ml_tag, chain_id) -> Dict:
    payload = {"pair": pair,"amount": amount,"direction": direction.lower(),
...
# ── Trade Execution ──────────────────────────────────────────────────────────
async def run_one_trade(pair, direction, expiry_min, amount, ml_label=None):
    ...
    try:
        ...
        # Immediate win → cancel MLs
        if profit > 0:
            if ml_label is None:
                cancel_task(ml1_task); cancel_task(ml2_task)
                reset_chain("Base WIN — killed ML1/ML2")
            elif ml_label == 1:
                cancel_task(ml2_task); reset_chain("ML1 WIN — killed ML2")
            elif ml_label == 2: reset_chain("ML2 WIN — chain complete")
            return
        elif ml_label == 2 and profit <= 0:
            reset_chain("ML2 LOSS — chain complete")
            return

        # Anchor MLs regardless, cancel if WIN later
        if ml_label is None:
            base_close = exec_time + timedelta(minutes=expiry_min)
            ml1_task = asyncio.create_task(schedule_leg(base_close+timedelta(seconds=ML1_GAP_S),1,chain_id))
        elif ml_label == 1:
            ml1_close = exec_time + timedelta(minutes=expiry_min)
            ml2_task = asyncio.create_task(schedule_leg(ml1_close+timedelta(seconds=ML2_GAP_S),2,chain_id))
    finally:
        executor_busy = False

# ── Scheduler ────────────────────────────────────────────────────────────────
async def schedule_leg(fire_dt: datetime, ml_label: Optional[int], cid: str):
    ...
    try:
        ...
        # Check for late WIN before firing
        for _ in range(15):  # 3s window
            ok,p,tag,peek_chain,closed_at = quick_peek(cid)
            if ok and p>0 and peek_chain==cid:
                reset_chain(f"{label} cancelled — WIN detected before fire")
                return
            await asyncio.sleep(0.2)
        amt = base_amount if ml_label is None else min(round(base_amount*(mg_mult**ml_label),2),MAX_STAKE)
        await run_one_trade(current["pair"],current["direction"],current["expiry_min"],amt,ml_label)
    except asyncio.CancelledError:
        print(f"[CANCEL] {label} cancelled."); return
...
async def handle_signal_from_text(text: str, msg_date=None):
    ...
    current.update({"active":True,"pair":force_otc(sig["pair"]),"direction":sig["direction"],
                    "expiry_min":sig["expiry_min"],"amount":base_amount,"chain_id":cid})
    ...
    t0=asyncio.create_task(schedule_leg(base_fire,None,cid)); scheduled_tasks.append(t0)
    return True
